<template>
  <v-container fluid grid-list-xl>
    <app-breadcrumbs :items="breadcrumbItems" icon="edit" title="Editar Exame"></app-breadcrumbs>
    <v-layout row wrap>
      <v-flex xs12>
  ...
  data() {
    return {
      ...
      breadcrumbItems: [
        ...
        { position: 3, text: "Editar Exame", disable: true}
      ],
    ...
    initialize() {
      axios.get("/exame/" + this.id).then(res => {
        ...
        this.breadcrumbItems.push({
          position: 2,
          text: this.titulo != undefined ? this.titulo : "Exame",
          disabled: false,
          href: "/coordenador/exames/" + this.id
          //text: this.titulo,
          //disabled: true
        }); 
      });
